...
def main(args):
    ...
    open(output_path + '/video_processed', 'w').write('\n'.join(chain.from_iterable(videos)))

    cmd = 'python ../ext_frames.py -f --hdfs --frame_need {0.frame_need} -o images -l video_processed -p {0.processes}'.format(
        args)
    ...
    run_system_command('tar -cf {0.output_name} output_path'.format(args))
